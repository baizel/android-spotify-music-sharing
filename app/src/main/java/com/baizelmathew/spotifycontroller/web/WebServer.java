...
public class WebServer extends NanoHTTPD {
    ...
    private OnFailSocketCallBack callBack;

    public static WebServer getInstance() throws NoRouteToHostException, NoSuchFieldError {
        if (server == null) {
     ...
     */
    private WebServer() throws NoRouteToHostException, NoSuchFieldError{
    ...
    public void startServer() throws IOException {
        if(!isInit) {
            throw new IOException("Server not initlisedd yet, make sure to call init()");
        }
        webSocket.start();
        this.start();
    ...
    public void init() throws NoRouteToHostException,NoSuchFieldError {
        if(!isInit) {
            if (callBack == null)
                throw new NoSuchFieldError("No WebSocketCallback set");
            webSocket = new WebSocket(getIPAddress(isIpv4), SOCKET_PORT).registerCallback(new WebSocketCallback() {
                ...
                public void onError(org.java_websocket.WebSocket conn, Exception ex) {
                    handleOnError(callBack, conn, ex);
                }
            });
            isInit = true;
        }
    ...
    private HashMap<String, String> refreshInjectionData() {
        ...
        injectionData.put("InitialState", playerInstance.getCachedPlayerState());
    ...
    public void setCallBack(OnFailSocketCallBack callBack) {
